...
def guard_url(url: str) -> str:
    p = urlparse(url)
    if p.scheme not in {"http", "https"}: raise UrlError("Only http/https allowed")
    if not p.hostname or not HOST_REGEX.match(p.hostname): raise UrlError("Invalid hostname")
    try:
        infos = socket.getaddrinfo(p.hostname, None)
        for info in infos:
            ip = info[4][0]
            if is_private_ip(ip): raise UrlError("Target resolves to private IP")
